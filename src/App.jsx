...
const isAmplifyConfigured = (() => {
  ...
  // Check for old format (aws_user_pools_web_client_id) - safely access properties with type casting
  if (outputs && typeof outputs === 'object' && 'aws_user_pools_web_client_id' in outputs && 'aws_user_pools_id' in outputs) {
    const clientId = outputs.aws_user_pools_web_client_id;
    const poolId = outputs.aws_user_pools_id;
    return typeof clientId === 'string' && !clientId.includes("REPLACE_WITH") &&
           typeof poolId === 'string' && !poolId.includes("REPLACE_WITH");
  }
  return false;
})();
